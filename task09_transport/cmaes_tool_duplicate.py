...
def write_code(file_id, code_file, tool_file, load_tool_file, plan_file, template_file='cmaes_tool_multiprocess.py'):
    ...
    tool = scene.add_entity(
        morph = gs.morphs.Mesh(
            file=\"{{}}/task09_transport_tools/{file_id}.obj\".format(project_path),
            scale=({s1}, {s2}, {s3}),
            pos=({p1}, {p2}, {p3}),
            euler=({e1}, {e2}, {e3}),
            fixed=False,
...
"""
    ...
    lines.append("        bottle_AABB = cup.get_AABB()")
...
project_path = os.path.dirname(os.path.join(os.path.abspath(__file__), '..'))
prefix = os.path.join(project_path, "task09_transport_tools")
...
for i in range(10):
    try:
        ...
        template_file = project_path + "/task09_transport/cmaes_tool_multiprocess.py"
