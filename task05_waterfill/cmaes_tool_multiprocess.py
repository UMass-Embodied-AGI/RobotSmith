...
def run_gs_env(env_id, input_queue, output_queue, project_path, img_steps, img_save_dir):
    ...
    def evaluate(x, cur_iter, vis, img_steps, img_save_dir):
        ...
        bottle_AABB = bottle.get_AABB()
        valid_id = (water_particles[:,0] > bottle_AABB[0][0]) and (water_particles[:,0] < bottle_AABB[1][0]) and \
                   (water_particles[:,1] > bottle_AABB[0][1]) and (water_particles[:,1] < bottle_AABB[1][1]) and \
                   (water_particles[:,2] > bottle_AABB[0][2]) and (water_particles[:,2] < bottle_AABB[1][2])
...
if __name__ == '__main__':
    ...
    log_dir = os.path.join(project_path, 'task05_waterfill', 'try')
