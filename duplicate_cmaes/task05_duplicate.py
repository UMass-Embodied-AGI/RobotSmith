...
"""
    ...
    lines.append("")
    lines.append("        water_particles = water.get_pos()")
    lines.append("        bottle_AABB = bottle.get_AABB()")
    lines.append("        valid_id = (water_particles[:,0] > bottle_AABB[0][0]) and (water_particles[:,0] < bottle_AABB[1][0]) and \\")
    lines.append("                   (water_particles[:,1] > bottle_AABB[0][1]) and (water_particles[:,1] < bottle_AABB[1][1]) and \\")
    lines.append("                   (water_particles[:,2] > bottle_AABB[0][2]) and (water_particles[:,2] < bottle_AABB[1][2])")
    lines.append("        penalty = -valid_id.sum()")
    lines.append("")
...
# write_code(file_id, code_file, tool_file, load_tool_file, plan_file, template_file='cmaes_tool_multiprocess.py'
...
code_file = prefix + "task05_cmaes.py"
tool_file = unity_prefix + "task05.obj"
load_tool_file = "task05.py"
plan_file = prefix + "task05_init_traj.txt"
template_file = "/home/xhrlyb/Projects/FuncAny/task05_waterfill/cmaes_tool_multiprocess.py"
file_id = '_blenderkit05'
...
tool_placement = get_tool_placement(prefix + 'task05.py')
